...
import { deleteFromDatabase, getFromDatabase, saveToDatabase } from './db'
...
function App() {
  ...
  async function backupData() {
    ...
    for (const item of items) {
      ...
      const locationPhoto = item.locationPhotoKey
  ...
  async function restoreData(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const backupItem of backup.items) {
        const { itemPhoto, locationPhoto, ...storedItem } = backupItem

        if (storedItem.itemPhotoKey && itemPhoto) {
          await saveToDatabase(storedItem.itemPhotoKey, itemPhoto)
        }

        if (storedItem.locationPhotoKey && locationPhoto) {
          await saveToDatabase(storedItem.locationPhotoKey, locationPhoto)
        }
